        ...
        <div class="">
            <div class="brd c-left list-holder village-list">
                ...
                <table id="list_table">
                    ...
                    $total_results = $db->getValue("blood_requests", "count(id)");
                    if ($total_results > 0) {
                        $perpage = 25;
                        ...
                        $sites = $db->rawQuery('SELECT * FROM blood_requests ORDER BY date DESC LIMIT ?,?', array($start, $perpage));

                        foreach ($sites as $sp) {
                            echo '<tr> <td> ' . $sp['name'] . ' </td>';
                            echo '<td> ' . getBloodGroup($sp['bgroup']) . '</td>';
                            echo '<td> ' . $sp['bags'] . '</td>';
                            echo '<td> ' . humanTiming($sp['date']) . '</td>';
                            echo '<td> ' . $sp['phone'] . '</td>';
