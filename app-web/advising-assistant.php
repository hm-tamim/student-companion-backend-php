    ...
    <div class="main">
        ...
        <div class="tabDetails bar1">
            <div id="tab1" class="tabContents">
                ...
                <form class="sForm" action="/app/advising-assistant.php" method="GET">
...
if (!empty($_GET['course'])) {
    ...
    echo '<a style="display: block; padding: 10px 15px; background: #023d6f; color: #fff; margin-bottom: 15px; border-radius: 5px;" href="/app/advising-assistant.php">Go back from Search <i class="fa fa-arrow-circle-o-left fa-2" style="float: right; font-size: 20px;"></i></a>';
...
if (isBot() == 1 || $islogged == 1 || $islogged == 0) {
    ...
    if (isset($_GET['course'])) {
        ...
        $idd = $db->rawQuery("SELECT * FROM advising WHERE " . implode(' OR ', $searchTermBits) . " ORDER BY id");
        ...
        ';
            ...
            if (!empty($fcourse)) {
                ...
                for ($i = 0; $i < sizeof($idd); $i++) {
                    ...
                    $matching = preg_match_all('!\d+!', $capacity, $matches);
                    if ($matches[0][0] == $matches[0][1])
                        $cfilled = 'class="cfilled"';
                    else if ($matches[0][1] - $matches[0][0] < 5)
...
<div class="tabDetails bar1" style="background: #fff;border-top-left-radius: 0px; border; padding: 20px;">
    ...
    <div class="lbody">
        ...
        <form class="sForm" action="addcourse-advising-assistant.php" method="GET">
            ...
            <input id="ssss" class="aa autoc disableComma" maxlength="7" style="text-transform: uppercase" type="search"
                   name="course"
...
<option value="' . $u . '"' . $selected . '>' . $uu . '</option>';
        ...
        <p id="comsug2" class="tips"><i class="fa fa-info-circle toggle-trigger"></i> Select <b>All</b> to include all
            sections of that course. </p>
...
<form action="delete-advising-assistant.php" method="GET">';
                ...
                $userCourses = $id[0]['courses'];

                $coursesArray = explode(PHP_EOL, $userCourses);

                for ($i = 0; $i < count($coursesArray); $i++) {


                    $extractCourse = explode(",", $coursesArray[$i]);

                    $insideCourse = trim($extractCourse[0]);

                    if ($extractCourse[1] != "0")
                        $insideSection = '<span>' . trim($extractCourse[1]) . '</span>';
                    else
                        $insideSection = '';


                    if (!empty($insideCourse))
                        echo '<button type="submit" name="course" value="' . $insideCourse . ',' . $extractCourse[1] . '">' . $insideCourse . ' ' . $insideSection . '<i class="fa fa-times fa-1" aria-hidden="true"></i></button>';
...
if (preg_match("/[a-z]/i", $userCourses)) {
    ...
    for ($i = 0; $i < count($coursesArray); $i++) {
        ...
        $insideSection = trim($extractCourse[1]);
