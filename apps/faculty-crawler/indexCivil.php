...
?>
    ...
    <form method="POST" action="indexCivil.php">
...
if (isset($_REQUEST['submit'])) {
    ...
    $length = sizeof($notices->find('div[class=single-faculty]'));

    $facultyArr = $notices->find('div[class=single-faculty]');
    ...
    for ($j = 0; i < $length; $j++) {
        ...
        $string = trim(strip_tags($row->find('a', 1)->innertext));


        $regex = "/(.*) &lbrack;(.*)&rbrack;/";
        if (preg_match($regex, $string, $match)) {
            $name = trim(cleanTags($match[1]));

            $initial = trim(cleanTags($match[2]));
        ...
        $ranks = trim($row->find('h6[class=small-text]', 0)->innertext);


        $parts = explode("<br/>", $ranks);
        ...
        $url = str_replace('http://institutions.northsouth.edu/cee/', '', $url);
        $image = str_replace('http://institutions.northsouth.edu/cee/', '', $image);
