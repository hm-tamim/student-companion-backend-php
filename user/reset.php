...
$title = "Reset Password - NSUer.Club";
...
if (isset($_POST['submit'])) {
    ...
    if (!isset($error)) {
        ...
        $stmt = $dbb->prepare('SELECT password, email FROM members WHERE email = :email');
        $stmt->execute(array(':email' => strtolower($_POST['email'])));
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        try {
            ...
            $stmt = $dbb->prepare("UPDATE members SET resetToken = :token, resetComplete='No' WHERE email = :email");
            $stmt->execute(array(
            ...
            //send email
            $to = $row['email'];
            $subject = "Password Reset - NSUer.Club";
            ...
            header('Location: /user/login.php?action=reset');
            exit;
